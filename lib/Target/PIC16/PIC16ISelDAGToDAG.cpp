...
namespace {
  struct PIC16ISelAddressMode {
    ...
    void dump() {
      errs() << "PIC16ISelAddressMode " << this << '\n';
...
namespace {
  ...
  public:
    PIC16DAGToDAGISel(PIC16TargetMachine &TM, CodeGenOpt::Level OptLevel)
    ...
    bool MatchAddress(SDValue N, PIC16ISelAddressMode &AM);
    bool MatchWrapper(SDValue N, PIC16ISelAddressMode &AM);
    bool MatchAddressBase(SDValue N, PIC16ISelAddressMode &AM);
...
FunctionPass *llvm::createPIC16ISelDag(PIC16TargetMachine &TM,
  ...
  return new PIC16DAGToDAGISel(TM, OptLevel);
}


/// MatchWrapper - Try to match PIC16ISD::Wrapper node into an addressing mode.
/// These wrap things that will resolve down into a symbol reference.  If no
/// match is possible, this returns true, otherwise it returns false.
bool PIC16DAGToDAGISel::MatchWrapper(SDValue N, PIC16ISelAddressMode &AM) {
...
bool PIC16DAGToDAGISel::MatchAddressBase(SDValue N, PIC16ISelAddressMode &AM) {
  // Is the base register already occupied?
  if (AM.BaseType != PIC16ISelAddressMode::RegBase || AM.Base.Reg.getNode()) {
    // If so, we cannot select it.
    return true;
  }

  // Default, generate it as a register.
  AM.BaseType = PIC16ISelAddressMode::RegBase;
  AM.Base.Reg = N;
  return false;
}

bool PIC16DAGToDAGISel::MatchAddress(SDValue N, PIC16ISelAddressMode &AM) {
  ...
  case PIC16ISD::Wrapper:
    if (!MatchWrapper(N, AM))
      return false;
    break;

  case ISD::FrameIndex:
    if (AM.BaseType == PIC16ISelAddressMode::RegBase
        && AM.Base.Reg.getNode() == nullptr) {
      AM.BaseType = PIC16ISelAddressMode::FrameIndexBase;
  ...
  case ISD::ADD: {
    PIC16ISelAddressMode Backup = AM;
...
/// match by reference.
bool PIC16DAGToDAGISel::SelectAddr(SDValue N,
                                    SDValue &Base, SDValue &Disp) {
  PIC16ISelAddressMode AM;
  ...
  Base = (AM.BaseType == PIC16ISelAddressMode::FrameIndexBase)
...
bool PIC16DAGToDAGISel::tryIndexedLoad(SDNode *N) {
...
bool PIC16DAGToDAGISel::tryIndexedBinOp(SDNode *Op, SDValue N1, SDValue N2,
...
void PIC16DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse()) {
      CurDAG->SelectNodeTo(Node, PIC16::ADD16ri, MVT::i16, TFI,
                           CurDAG->getTargetConstant(0, dl, MVT::i16));
      return;
    }
    ReplaceNode(Node, CurDAG->getMachineNode(
                          PIC16::ADD16ri, dl, MVT::i16, TFI,
                          CurDAG->getTargetConstant(0, dl, MVT::i16)));
    return;
  }
  case ISD::LOAD:
  ...
  case ISD::ADD:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::ADD8rm_POST, PIC16::ADD16rm_POST))
      return;
  ...
  case ISD::SUB:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::SUB8rm_POST, PIC16::SUB16rm_POST))
      return;
  ...
  case ISD::AND:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::AND8rm_POST, PIC16::AND16rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             PIC16::AND8rm_POST, PIC16::AND16rm_POST))
      return;
  ...
  case ISD::OR:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::OR8rm_POST, PIC16::OR16rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             PIC16::OR8rm_POST, PIC16::OR16rm_POST))
      return;
  ...
  case ISD::XOR:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::XOR8rm_POST, PIC16::XOR16rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             PIC16::XOR8rm_POST, PIC16::XOR16rm_POST))
      return;
