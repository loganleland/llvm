...
void PIC16DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse()) {
      CurDAG->SelectNodeTo(Node, PIC16::ADD8ri, MVT::i8, TFI,
                           CurDAG->getTargetConstant(0, dl, MVT::i8));
      return;
    }
    ReplaceNode(Node, CurDAG->getMachineNode(
                          PIC16::ADD8ri, dl, MVT::i8, TFI,
                          CurDAG->getTargetConstant(0, dl, MVT::i8)));
    return;
  }
  case ISD::LOAD:
  ...
  case ISD::ADD:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::ADD8rm_POST, PIC16::ADD8rm_POST))
      return;
  ...
  case ISD::SUB:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::SUB8rm_POST, PIC16::SUB8rm_POST))
      return;
  ...
  case ISD::AND:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::AND8rm_POST, PIC16::AND8rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             PIC16::AND8rm_POST, PIC16::AND8rm_POST))
      return;
  ...
  case ISD::OR:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::OR8rm_POST, PIC16::OR8rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             PIC16::OR8rm_POST, PIC16::OR8rm_POST))
      return;
  ...
  case ISD::XOR:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        PIC16::XOR8rm_POST, PIC16::XOR8rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             PIC16::XOR8rm_POST, PIC16::XOR8rm_POST))
      return;
