...
void PIC16FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(PIC16::PUSH16r))
      .addReg(PIC16::FP, RegState::Kill);

    // Update FP with the new base value...
    BuildMI(MBB, MBBI, DL, TII.get(PIC16::MOV16rr), PIC16::FP)
      .addReg(PIC16::SP);
  ...
  if (NumBytes) { // adjust stack pointer: SP -= numbytes
    ...
    if (NumBytes) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL, TII.get(PIC16::SUB16ri), PIC16::SP)
...
void PIC16FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  switch (RetOpcode) {
  case PIC16::RET:
  case PIC16::RETI: break;  // These are ok
  default:
  ...
  if (hasFP(MF)) {
    ...
    // pop FP.
    BuildMI(MBB, MBBI, DL, TII.get(PIC16::POP16r), PIC16::FP);
  } else
  ...
  if (MFI->hasVarSizedObjects()) {
    BuildMI(MBB, MBBI, DL,
            TII.get(PIC16::MOV16rr), PIC16::SP).addReg(PIC16::FP);
    if (CSSize) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL,
                TII.get(PIC16::SUB16ri), PIC16::SP)
        .addReg(PIC16::SP).addImm(CSSize);
      // The SRW implicit def is dead.
      MI->getOperand(3).setIsDead();
    }
  } else {
    ...
    if (NumBytes) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL, TII.get(PIC16::ADD16ri), PIC16::SP)
        .addReg(PIC16::SP).addImm(NumBytes);
      // The SRW implicit def is dead.
      MI->getOperand(3).setIsDead();
    }
