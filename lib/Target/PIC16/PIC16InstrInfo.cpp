...
void PIC16InstrInfo::storeRegToStackSlot(MachineBasicBlock &MBB,
  ...
  if (RC == &PIC16::GR16RegClass)
    BuildMI(MBB, MI, DL, get(PIC16::MOV16mr))
      .addFrameIndex(FrameIdx).addImm(0)
      .addReg(SrcReg, getKillRegState(isKill)).addMemOperand(MMO);
  else if (RC == &PIC16::GR8RegClass)
    BuildMI(MBB, MI, DL, get(PIC16::MOV8mr))
      .addFrameIndex(FrameIdx).addImm(0)
      .addReg(SrcReg, getKillRegState(isKill)).addMemOperand(MMO);
  else
...
unsigned PIC16InstrInfo::RemoveBranch(MachineBasicBlock &MBB) const {
  ...
  while (I != MBB.begin()) {
    ...
    if (I->isDebugValue())
      continue;
    if (I->getOpcode() != PIC16::JMP &&
        I->getOpcode() != PIC16::JCC &&
        I->getOpcode() != PIC16::Br &&
        I->getOpcode() != PIC16::Bm)
      break;
    // Remove the branch.
    I->eraseFromParent();
    I = MBB.end();
...
ReverseBranchCondition(SmallVectorImpl<MachineOperand> &Cond) const {
  ...
  PIC16CC::CondCodes CC = static_cast<PIC16CC::CondCodes>(Cond[0].getImm());
...
bool PIC16InstrInfo::isUnpredicatedTerminator(const MachineInstr &MI) const {
  if (!MI.isTerminator())
    return false;
...
bool PIC16InstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (I != MBB.begin()) {
    ...
    assert(I->getOpcode() == PIC16::JCC && "Invalid conditional branch");
    PIC16CC::CondCodes BranchCode =
      static_cast<PIC16CC::CondCodes>(I->getOperand(1).getImm());
    if (BranchCode == PIC16CC::COND_INVALID)
    ...
    PIC16CC::CondCodes OldBranchCode = (PIC16CC::CondCodes)Cond[0].getImm();
...
unsigned PIC16InstrInfo::InsertBranch(MachineBasicBlock &MBB,
                                       MachineBasicBlock *TBB,
                                       MachineBasicBlock *FBB,
  ...
  assert((Cond.size() == 1 || Cond.size() == 0) &&
         "PIC16 branch conditions have one component!");

  if (Cond.empty()) {
    // Unconditional branch?
    assert(!FBB && "Unconditional branch with multiple successors!");
    BuildMI(&MBB, DL, get(PIC16::JMP)).addMBB(TBB);
    return 1;
  }

  // Conditional branch.
  unsigned Count = 0;
  BuildMI(&MBB, DL, get(PIC16::JCC)).addMBB(TBB).addImm(Cond[0].getImm());
  ++Count;

  if (FBB) {
    // Two-way Conditional branch. Insert the second branch.
    BuildMI(&MBB, DL, get(PIC16::JMP)).addMBB(FBB);
    ++Count;
  }
  return Count;
}
